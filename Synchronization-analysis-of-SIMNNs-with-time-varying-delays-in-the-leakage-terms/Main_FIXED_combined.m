...
sol = ddesd(@Equations3,@Lambda_delay2,init,[t0, tf]);
fprintf("SOLVED ------ \n")
%%
styles2 = {'-b','--r'};
styles = {'-b','--g'};
...
for i = 1:m
    for j = 1:n
        ...
        dist = strcat(dist,"_FIXED_combined");
...
saveas(figure(201), strcat(header, '_FIXED_error'), 'png');
%%
figure(101)
plot3(sol.y(m*n+2,:), sol.y(m*n+3,:), sol.y(m*n+1,:));
saveas(figure(101), strcat(header, '_FIXED_phase_plot'), 'png');
%%
END = clock;